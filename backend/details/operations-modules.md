...
- `ops`: 换电、调度、维修等现场任务，包含派单、抢单、状态流转和事件时间线。
...
- 同一车辆只能存在一个未结束任务，避免多个班组重复到场。
- `version` 乐观锁保护指派、释放、开始、完成和取消，状态变化时返回 `409`。
- 任务开始后，调度和回收将车辆切为 `DISPATCHING`，其余任务切为 `MAINTENANCE`；完成或取消后恢复 `OPERATING`。
- 事件表保留创建、抢单、指派、释放、开始、完成和取消轨迹，管理员可追溯领取人与处理结果。
