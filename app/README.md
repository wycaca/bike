...
- 管理人员：运营总览、车辆地图与查询、任务派单、验收、异常闭环、自动规则和批量任务。
- 运维人员：任务池抢单、我的作业、路线优化、车辆地图与查询、定位、扫码、照片凭证和异常上报。

车辆页默认显示高德地图，可切换在线状态、查看聚合数量和异常数量；放大聚合点后可点击单车标记查看电量、连接状态、控制器状态和坐标。地图与列表可通过页面顶部的分段控件切换。

管理人员的运营总览第一屏展示截至昨日或最近结算日的经营数据，包括单日和当月累计净收入、有效订单、单车日均骑行次数（RpD）、单车日均收入（RevPVD）、单均收入、优惠率、退款率及近七日趋势；任务执行数据位于经营指标之后。
